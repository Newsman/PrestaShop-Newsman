...
class NewsmanRemarketing extends Module
{
        ...
        if (Validate::isLoadedObject($order) && $order->getCurrentState() != (int) Configuration::get('PS_OS_ERROR')) {
            ...
            if ($order->id_customer == $this->context->cookie->id_customer) {
                ...
                $transaction = [
                    ...
                    'shipping' => $order->total_shipping,
                    'tax' => $order->total_paid_tax_incl - $order->total_paid_tax_excl,
                    'url' => $this->context->link->getModuleLink('newsmanremarketing', 'ajax', [], true),
                    'customer' => $order->id_customer];
    ...
    public function hookHome()
    {
        ...
        if ($this->isModuleEnabled('homefeatured')) {
            ...
            $home_featured_products = $this->wrapProducts($category->getProducts(
        ...
        if ($this->isModuleEnabled('blockbestsellers') && (!Configuration::get('PS_CATALOG_MODE')
            ...
            $ga_homebestsell_product_list = $this->wrapProducts(ProductSale::getBestSalesLight((int) $this->context->language->id, 0, 8), [], true);
